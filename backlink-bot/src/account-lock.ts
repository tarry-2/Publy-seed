...
export function acquireAccountLock(accountId: string, owner: string): AccountLock {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      fs.writeFileSync(fd, payload, "utf8"); fs.closeSync(fd);
      return { ok: true, release: () => {
        try { const current = JSON.parse(fs.readFileSync(file, "utf8")); if (current.token === token) fs.unlinkSync(file); } catch {}
      } };
    } catch (error: any) {
