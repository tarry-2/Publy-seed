...
const UB="6f4612398e929e09a365__cr.kr", PASS="d62608f358fe9282";
...
async function visit(n){
  const ctx=await chromium.launchPersistentContext("",{channel:"chrome",headless:false,userAgent:UA,viewport:{width:412,height:915},isMobile:true,hasTouch:true,deviceScaleFactor:3,locale:"ko-KR",
    proxy:{server:"http://gw.dataimpulse.com:823",username:UB,password:PASS}});  // 823 rotating
  ...
  try{
    ...
    if(!gates.length){ note="게이트없음(검색차단?)"; }
    else{
      ...
      note=blk(b)?"🚫차단":/NAVER 로그인/.test(t)?"🔒로그인":buy?`🛒상품(${t.slice(0,18)})`:"△len"+b.length;
    }
  }catch(e){ note="err:"+(e.message||"").slice(0,25); }
