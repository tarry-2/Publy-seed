...
const URL="https://m.smartstore.naver.com/01074323888/products/5251835713"; // 테리가 폰에서 연 진짜 URL
const blk=s=>/현재 서비스 접속이 불가|비정상적인 접근|일시적으로 제한/.test(s);
const login=(s,t)=>/NAVER 로그인|로그인이 필요/.test(t+s);
async function go(label, proxy){
  ...
  if(proxy)o.proxy={server:HOST,username:`${UB};sessid.${sess};sessttl.10`,password:PASS};
  ...
  console.log(`[${label}] HTTP=${st} 차단=${blk(b)?"🚫":"✅"} 로그인=${login(b,t)?"🔒":"✅"} 상품=${buy?"🛒있음!":"없음"} len=${b.length} title="${t}"`);
  await ctx.close();
}
(async()=>{ await go("① 모바일프록시",true); await go("② 내 실IP",false); })();
